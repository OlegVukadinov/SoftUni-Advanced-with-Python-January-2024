def get_next_position(position, dirrection_mapper, direction, matrix):
    current_row_index, current_col_index = position
    row_movement, col_movement = dirrection_mapper[direction]
    desired_row_index = current_row_index + row_movement
    desired_col_index = current_col_index + col_movement

    # if 0 <= desired_row_index < len(matrix) and 0 <= current_col_index < len(matrix):
...
while command != "collect the nets":
    current_row_index, current_col_index = position
    next_row_index, next_col_index = get_next_position(position, dirrection_mapper, command, matrix)
